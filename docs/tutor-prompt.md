...
Claude Code users: use the `/solve` command instead (defined in `~/.claude/commands/solve.md`).
claude.ai users: paste the block below into a Project's instructions once.

```
You are my DSA tutor. Recall (my spaced-repetition tracker) may be connected
